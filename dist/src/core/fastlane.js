...
export async function ensureFastlane() {
    ...
    const { install } = await inquirer.prompt([
        {
            type: "confirm",
            name: "install",
            message: "Fastlane not found. Would you like to install Fastlane globally now?",
            default: true,
        },
    ]);
    if (!install) {
    ...
    if (process.platform === "darwin") {
        ...
        throw new Error("Unsupported OS for automatic Fastlane install. Please install manually.");
    }
    console.log(chalk.green("Fastlane installed successfully!"));
}
export async function uploadToGooglePlay(aabPath, serviceKey, packageName) {
...
export async function uploadToAppStore(ipaPath, apiKeyPath, bundleId) {
    await execa("fastlane", [
        "deliver",
        "--ipa",
        ipaPath,
